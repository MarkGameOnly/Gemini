...
def create_invoice(user_id, amount=1):
    try:
        headers = {
        ...
        payload = {
            "asset": "USDT",
            "amount": amount,
            "description": "Подписка Gemini на 1 месяц",
            "hidden_message": "Спасибо за оплату!",
            "expires_in": 300,
            "payload": str(user_id)
        }
        response = requests.post("https://pay.crypt.bot/api/createInvoice", headers=headers, json=payload)
        return response.json()["result"]["pay_url"]
    except Exception as e:
        print("Ошибка создания счёта:", e)
        return None
...
def action_buttons(user_id):
    ...
    if invoice_url:
        buttons.append([InlineKeyboardButton("💳 Купить подписку $1", url=invoice_url)])
